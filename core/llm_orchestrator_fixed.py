...
class LLMOrchestrator:
    ...
    def _initialize_models(self) -> Dict[str, Any]:
        ...
        if self._check_ollama():
            ...
            for model in ollama_models:
                models[f"ollama:{model}"] = {"provider": ModelProvider.OLLAMA, "model": model, "local": True}
            logger.info(f"Found {len(ollama_models)} Ollama models")

        # Cloud models (with API keys)
        if os.getenv("OPENAI_API_KEY"):
            models["openai:gpt-4"] = {"provider": ModelProvider.OPENAI, "model": "gpt-4-turbo-preview", "local": False}
        ...
        if not models:
            ...
            models["mock:test"] = {"provider": "mock", "model": "test", "local": True}
    ...
    def _check_ollama(self) -> bool:
        ...
        for path in ollama_paths:
            try:
                result = subprocess.run([path, "list"], capture_output=True, text=True, timeout=5)
    ...
    def _get_ollama_models(self) -> List[str]:
        ...
        try:
            result = subprocess.run([self.ollama_path, "list"], capture_output=True, text=True, timeout=5)
...
If no tools needed, omit tool_call. Keep it simple."""
    ...
    def _parse_execution_plan(self, llm_response: str, user_input: str) -> ExecutionPlan:
        ...
        try:
            ...
            return ExecutionPlan(
                ...
                steps=(
                    steps if steps else [ExecutionStep(step_id="1", description="Respond directly", prompt=user_input)]
                ),
            ...
            return ExecutionPlan(
                ...
                steps=[ExecutionStep(step_id="1", description="Direct response", prompt=user_input, model="primary")],
    ...
    async def _query_llm(self, model_key: str, prompt: str) -> str:
        ...
        if model_info["provider"] == "mock":
            ...
            return await self._query_anthropic(model_info["model"], prompt)
        else:
    ...
    def _mock_response(self, prompt: str) -> str:
        ...
        if "list" in prompt.lower() and "file" in prompt.lower():
                    ...
                    "plan": [
                        {
                            ...
                            "tool_call": {"tool_name": "filesystem", "parameters": {"operation": "list", "path": "."}},
                        }
                    ],
            ...
            return json.dumps(
                {
                    "goal": "Respond",
                    "plan": [{"step_id": "1", "description": "Direct response", "prompt": prompt}],
